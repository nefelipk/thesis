...
\begin{document}
	\Large\textbf{\centerline{1. Introduction}}
	\normalsize\newline\newline
	...
	\newpage
	\Large\textbf{\centerline{2. Background}}
	\newline\newline
	
	\large\textbf{2.1 A few words}
	\normalsize\newline
	...
	1. Traversability estimation algorithms that can be improved from 
	experience/examples.
	
	2. Methods for collecting the data needed by the algorithm above, from 
	the sensory input that is available to the robot. (The input we have does not 
	directly map to positive/negative decision).
	
	3. Goals of operation. There might be an explicit goal to achieve or it 
	...
	\large\textbf{2.2 Traversability estimation algorithms}
	\normalsize\newline
	...
	[6]. This survey states that most commonly, traversability analysis is treated 
	as a binary classification problem, i.e. distinguishing traversable from 
	non-traversable terrain [1], [4], [5]. It also declares that one of he most popular and extensively used approaches 
