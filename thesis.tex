...
%\usepackage[english, greek]{babel}	% (with PdfLaTex)
...
\usepackage[font={small,bf}]{subcaption}
\usepackage{multirow}
\graphicspath{ {./images/} }
...
\usepackage[a4paper,top=2cm,bottom=2cm,bindingoffset=0.5cm,left=2cm,right=2cm,
headheight=15pt,headsep=1.25cm,footskip=1.25cm,includehead,includefoot]{geometry}
...
%
% Add Greek support (with LuaLaTex)
%
\usepackage{fontspec}
\usepackage{xunicode}
\usepackage{xltxtra}
...
\fancyhf{}
\fancyhead[LE,LO]{\headerfont Autonomic tackling of unknown obstacles in navigation of robotic platform}
\fancyfoot[LE,LO]{\headerfont N. Prokopaki Kostopoulou}
\fancyfoot[RE,RO]{\headerfont \thepage}
...
%
% Configure list of contents printing and alignment
%
...
\renewcommand{\cftparaleader}{\cftdotfill{\cftsecdotsep}}
\setlength{\cftparaindent}{\cftsubsubsecindent}
...
\begin{document}
	%
	% Set font Arial (build with LuaLaTex)
	%
...
%	\selectlanguage{english}
	...
	
	{
		...
		% (thanks to https://tex.stackexchange.com/questions/5787/table-of-contents-with-page-style-empty)
	}
	...
	The first layer in a convolutional neural network is always a \term{convolutional layer}. 
	The best way to explain a convolutional layer is to imagine a flashlight 
	...
	\term{kernel}) and the region that it is shining over is called \term{receptive field}. 
	This filter is also an array of numbers called \term{weights} or 
	...
	the result is an 28 $\times$ 28 $\times$ 1 array of numbers, called an \term{activation map} 
	or \term{feature map}. Using more filters makes possible to preserve the spatial 
	...
	\begin{figure}[h!]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\draw[red,thick] (7.3,1.6) circle (0.3) ;
			\end{tikzpicture}
			\caption{Image resulting from \acronym{psp}}
	...
	\begin{figure}[h!]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\node[anchor=south west,inner sep=0] at (0,0) 
			{\includegraphics[width=\textwidth]{28_57_0000t_psp}};
			\draw[red,thick] (6.0,2.0) rectangle (1.5,0.03) ;
			\end{tikzpicture}
		...
		\end{subfigure}
		\caption{Two minimized images (one traversable and one non-traversable) so that the 
			robotic platform fits marginally within their frame.}
		\label{fig:mini}
	\end{figure}
	...
	around it, while at the same time gather geometric information about it, like 
	height or depth. We believe that with a combination like this, the results would 
	...
	our program could be ready to be used on an actual robotic platform to explore 
