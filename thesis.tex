...
\begin{document}
	...
	In this chapter we describe our primary thoughts and then our actual strategy on how to 
	...
	convolutional networks, at least on the cases we saw them used, do semantic segmentation 
	only on foreground and ignore the background (Figure~\ref{fig:fcn}). In order for us to 
	be able to decide terrain traversability, whole scene segmentation is necessary.
	\\
	...
	as mentioned before, ignore the background. But pixel appearance features allow to perform 
	well on classifying (amorphous) background classes. \acronym{ade20k} categorizes semantic 
	classes of the scenes into stuff (sky, road, building, etc), foreground objects (car, tree, 
	sofa, etc), and object parts (car wheels and door, people head and torso, etc).
	\\
	...
	Many deep learning architectures (like fully connected networks mentioned above) have 
	been proposed for image segmentation. But Google’s DeepLab model \citep{Chen} has given 
	the best results till date \cite{Pal}. This, together with the fact that we managed to 
	find DeepLab pre-trained on \acronym{ade20k}, is why we finally chose to use it in this 
	thesis.
	\\
