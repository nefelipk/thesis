...
\begin{document}
	...
	\par
	For a brief description of the most important models mentioned above, \citet{Krizhevsky_alexnet} 
	...
	we see in a visual scene is called \term{visual recognition}. Such tasks are object 
	classification and localization, which concern one object. In computer vision, 
	visual recognition has enjoyed some great success in recent years, particularly 
	in single object categorization (i.e., classification and localization), like in 
	the example with the cat and the dog, mentioned above in Section~\ref{sec:bg:data:neural}. 
	...
	detection or segmentation), let alone reach a complete understanding of visual 
	scenes.
	\\
	
	
	In this thesis, our first goal is for the robot to be able to autonomously 
	navigate in natural environments. So, the aim is on total scene segmentation or 
	object detection rather than object recognition and localization. Although humans 
	perform scene segmentation with apparent ease, automatic scene segmentation is 
	a very challenging problem \citep{Wang}. 
