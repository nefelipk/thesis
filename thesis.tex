...
\begin{document}
	...
	Therefore, the sidewalk (and, as matter of fact, the color red) has 80\% chance of 
	being traversable.
	\\
	...
	\begin{enumerate}
		\item The first method determines the probability of an image to be penetrable, as the average of the traversability percentages for each color contained within. (The first method determines that the probability of an image to be penetrable is the average of the traversability percentages for each color contained therein.)
		Figure~\bl{\ref{fig:meth}}, which contains red, purple and yellow, has a probability of 51.67\% being traversable.
		\[ (80\% + 30\% + 45\%) \div 3 = 51.67\% \]
		\item The second method uses the number of times each color was found in traversable and in non-traversable images. It assumes that the likelihood of an image being traversable is in direct dependence of the number of times each color found within, is penetrable. In other words, it specifies that the traversability percentage of an image, is the weighted average of the traversability percentage of all the colors within it. 
	...
	in other cases the results may concur.
	\\\\ 
	...
	\begin{itemize}
		...
		\item In column 3 is the traversability percentage of each label, after the system is 
		trained with the images in the remaining bag files (after training, how likely is for 
	...
	images? We can not limit the number of the training images in a way so that traversable 
	and non-traversable images are equal in number. In the real life, the environment is 
	not evenly distributed in penetrable and impenetrable areas, so why to be here? Some 
