...
\begin{document}
	...
	as shown in Figure~\bref{fig:clas}.
	\\
	...
	within the image, as shown in Figure~\bref{fig:det}. Also, it is not necessary to draw 
	...
	to be segmented separately. For example see Figure~\bref{fig:segm}. As one can imagine, 
	...
	\acronym{pascal voc} are shown in Figure~\bref{fig:fcn}.
	\par
	...
	and \acronym{coco} \citep{Lin},	along with the rapid development of the deep convolutional 
	...
	segmentation only on foreground and ignore the background (Figure~\bref{fig:fcn}). In 
	...
	trained on each of the three datasets are depicted in Figure~\bref{fig:in} for indoor 
	inputs, and Figure~\bref{fig:out} for outdoor inputs.
	\\
	...
	image semantic segmentation. These images, as shown in Figure~\bref{fig:segm}, consist 
	...
	bag files for testing and the rest for training. In Table~\bref{table:folds_bags} one 
	can see how the bag files split for each fold. And in Table~\bref{table:folds_images} 
	...
	are shown in Table~\bref{table:1st}. And the results of the second method in Table~\bref{table:2nd}. 
	In both tables it is quite clear that the PSP has clearly/without doubt/undoubtedly/patently better results than the other two models, both individually and globally.
	\\
	...
	In Table~\bref{table:psp} we gathered the results from both methods when using data 
	...
	training set are much more that the traversable ones, as shown in Table~\bref{table:folds_images}.
	\\
	...
	\begin{table}[h!]
		...
		\begin{tabular}{|c|c|c|c|c||c|}
			\hline
			\multicolumn{6}{|c|}{1st method with \acronym{psp}}\\
			\hline
			\multirow{2}{*}{Fold} & \multicolumn{2}{|c|}{Traversable images} & \multicolumn{2}{|c||}{Non-traversable images} & \multirow{2}{*}{Average (\%)}\\
	...
	In Table~\bref{table:psp} one can easily observe that fold 1 has the worst results 
	...
	\begin{table}[h!]
		\begin{subtable}[h]{\textwidth}
			...
			\begin{tabular}{|c|c|c|}
				\hline
				\multicolumn{3}{|c|}{Fold 1}\\
				\hline
				Class label & Instances & Traversability (\%)\\
				\hline\hline
				grass & 52 & 100\\
				\hline
				person & 47 & 30\\
			...
			\label{table:trav.f1}
		\end{subtable}
		\caption{Traversable images - Labels, number of their instances and their 
			traversability percentage. (continuing in next page)}
	...
	\begin{table}[h!]
		...
		\begin{subtable}[h!]{\textwidth}
			...
			\label{table:trav.f3}
		\end{subtable}
		\caption{(continued) Traversable images - Labels, number of their instances and 
			their traversability percentage. (continuing in next page)}
	\end{table}
	...
	\begin{table}[h!]
		...
		\end{subtable}
		\caption{(continued) Traversable images - Labels, number of their instances and 
			their traversability percentage.}
	\end{table}
	
	In Tables~\ref{table:trav.folds} we have gathered all class labels appearing in each 
	one of the traversable	images for evaluation. Each subtable contains the information 
	for each fold. For every label mentioned we keep track of two numbers. The number of 
	its appearances in the current fold (in how many images is the color participating?). 
	And its traversability percentage, given the training images of the remaining rosbags 
	(after training, how likely is that this color is traversable?). 
	(after training, how likely is for this color to be traversable?). 
	\\
	
	Have in mind that in this work we do not care about the annotations representing each 
	color within the image. These labels are available to facilitate reading. They do not 
	always represent what is depicted in the image. But this thesis is not about judging 
	the proper functioning of the neural network. It is about estimating the traversability 
	of images resulting from the nervous network. (But we are not here to judge the proper 
	functioning of the neural network.)
	\\
	
	As mentioned before fold 1 seems to have/be having worse results than the other folds. 
	\\
	...
	\begin{table}[h!]
		...
		\begin{subtable}[h]{\textwidth}
			...
			\label{table:obst.f2}
		\end{subtable}
		\caption{Non-traversable images - Labels, number of their instances and their 
			traversability percentage. (continuing in next page)}
	...
	\begin{table}[h!]
		...
		\begin{subtable}[h!]{\textwidth}
			...
			\label{table:obst.f4}
		\end{subtable}
		\caption{(continued) Non-traversable images - Labels, number of their instances 
			and their traversability percentage. (continuing in next page)}
	\end{table}
	...
	\begin{table}[h!]
		...
		\end{subtable}
		\caption{(continued) Non-traversable images - Labels, number of their instances 
			and their traversability percentage.}
	\end{table}
	...
	use their work. In Table~\bref{table:refs} we give some \acronym{url}s and 
	...
	In Table~\bref{table:dataset} one can find some of the available datasets 
	used for neural network training and evaluation.
	\\
