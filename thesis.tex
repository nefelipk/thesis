...
\begin{document}
	...
	\todo[size={{\scriptsize}}]{Did SSD and YOLO won some competition? Say a few words about them..}
	...
	of interest to locate it within the image. Also, it is not necessary to draw just 
	one bounding box in the object detection case. There could be many bounding boxes 
	...
	a method called \term{\acronym{r-cnn}: Regions with \acronym{cnn} features}. A selective 
	search \citep{Uijlings} is used to extract just 2000 regions from the image. The authors 
	called them region proposals. These 2000 candidate region proposals are warped into a 
	square and fed into a convolutional neural network (in this case VGGNet) that acts as a 
	feature extractor. The extracted features are then used to classify the presence of the 
	object within that candidate region proposal and to refine the boundary box. The main 
	disadvantage here is that it still takes a huge amount of time to train the network as 
	there 2000 region proposals per image to classify. Also, the selective search algorithm 
	is a fixed algorithm. Therefore, no learning is happening at that stage, something that 
	could lead to generating bad candidate region proposals. That is why \term{Fast 
	\acronym{r-cnn}} and \term{Faster \acronym{r-cnn}} followed up.
	\par
	...
	convolutional network (a VGGNet pre-trained model) on input image only once and 
	...
	\todo[size={{\scriptsize}}]{A few words about FCNs..}
	...
	high-level task that paves the way towards complete scene understanding. The 
	...
	\term{Fully Convolutional Neural Networks (\acronym{fcn}s)} \citep{Long}.
	\\\\
	...
	\todo[size={{\scriptsize}}]{How to add citation for site? Or book..?}
	...
	\todo[size={{\scriptsize}}]{COCO for object detection, not only classification? SSD? YOLO? R-CNN? (Mask R-CNN)}
	...
	\par
	\todo[size={{\scriptsize}}]{A few more things about Tai...}
	...
	\todo[size={{\scriptsize}}]{A few more things about Sermanet and Hadsell...}
	...
	\citep{Tai}.
	\todo[size={{\scriptsize}}]{Why can't we use Tai, Sermanet or Hadsell? not available code}
	\todo[size={{\scriptsize}}]{Also other papers mentioned above \& MAYBEs on papers file}
	...
	\todo[size={{\scriptsize}}]{Hirose GONet: trained for indoors}
	
	\todo[size={{\scriptsize}}]{Fully Convolutional Networks?}
	\todo[size={{\scriptsize}}]{FCN: need training ---> \url{https://medium.com/nanonets/how-to-do-image-segmentation-using-deep-learning-c673cc5862ef}}
