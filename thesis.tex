...
\begin{document}
	...
	\todo[size={{\scriptsize}}]{Probably should say some things about SSD, YOLO, etc. And the one we will use..}
	
	Also, many papers have been published regarding robot navigation approaches that 
	use already existing deep neural networks. Some classify the area in front of the 
	robot for traversability \citep{Sermanet, Hadsell}. In others the concept of deep 
	neural networks is further extended to not only perception but also decision-making 
	\citep{Tai}.
	\todo[size={{\scriptsize}}]{\citet{Tai, Sermanet, Hadsell} not available code}
	\\\\
	...
	navigate in natural environments. To do so, we could use a pre-trained neural 
	network in order to be able to distinguish traversable from non-traversable 
	terrain. 
	\\
	
	The first thought was to use one of the most-well known models from \acronym{ilsvrc}. 
	...
	work, were the worst case scenario. For example, these networks after being 
	fed with an image of a tree gave as predictions with the largest probabilities 
	kinds of seeds, like lemons. This probably happened because the networks concentrated 
	to only the one part of the whole image that was most recognizable by them. 
	Finding out what the predictions with smaller probabilities were did not help. 
	As the networks kept predicting they got desperate are started giving all sorts 
	of provisions. 
	\par
	...
	The next idea was to use part of the work that \citet{Tai} did in order to explore 
	an unknown environment. They used a structure that fuses several convolutional 
	neural network layers with decision-making process.
	\todo[size={{\scriptsize}}]{Why can't we use Tai? Something like Sermanet or Hadsell?}
	
	\todo[size={{\scriptsize}}]{Write all efforts and ideas, whether successful or not}
	
	

	\todo[size={{\scriptsize}}]{Maybe Fully Convolutional Networks?}
	\todo[size={{\scriptsize}}]{Or R-CNN for object detection, not only classification? (Mask R-CNN)}
	\todo[size={{\scriptsize}}]{COCO for object detection? SSD? YOLO?}
	...
	\todo[size={{\scriptsize}}]{ADE20K: scene parsing, dataset that has densely annotated images}
